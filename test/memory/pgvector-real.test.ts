...
describe.skipIf(!URL)("pgvector stator — real Postgres", () => {
  ...
  it("scopes tiers/sessions and supersedes facts identically on a live server", async () => {
    ...
    await store.writeFacts([{ entity: "user", role: "note", filler: "ephemeral", key: "user:note", is_current: true, tick: 0, tier: "short", session: "s1" }]);
    await store.writeFacts([{ entity: "user", role: "pref", filler: "tabs", key: "user:pref", is_current: true, tick: 0, tier: "long", session: "s1" }]);
    // supersession
  ...
  it("ranks turns by cosine similarity via a real hnsw index", async () => {
    await reset();
    const store = await PgVectorStore.create({ url: URL });
    await store.addTurn("the config parser parseConfig reads yaml");
    await store.addTurn("the payment gateway charges a card");
    await store.addTurn("kubernetes schedules pods onto nodes");
    await store.flush();
  ...
  it("hydrates facts + the run tape across a restart (a fresh pod, same DB)", async () => {
    ...
    await s1.writeFacts([{ entity: "user", role: "pref", filler: "vim", key: "user:pref", is_current: true, tick: 0, tier: "long" }]);
    await s1.addTurn("a durable turn");
    await s1.shutdown();

    const s2 = await PgVectorStore.create({ url: URL });
    expect((await s2.lookupFact("user", "pref"))?.filler).toBe("vim"); // hydrated
    expect(await s2.turns()).toContain("a durable turn");
    expect(await s2.sessionOrdinal("sess-A")).toBe(0); // sessions hydrated
    await s2.shutdown();
  });
});
