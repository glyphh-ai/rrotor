...
describe("pgvector stator — tier/session parity", () => {
  ...
  it("runs the same closed ops as the pure engine (supersession-correct)", async () => {
    ...
    expect(store.query("prev", { person: "ada", slot: "city" }).rows[0]?.filler).toBe("London");
    await store.flush();
  });
});
...
describe("pgvector stator — durable hydrate across a restart", () => {
  it("a fresh store over the same database rebuilds facts, turns and sessions", async () => {
    ...
    s1.writeFacts([{ entity: "user", role: "pref", filler: "tabs", key: "user:pref", is_current: true, tick: 0, tier: "long" }]);
    s1.addTurn("we decided to use pgvector for the durable stator");
    ...
    expect(s2.lookupFact("user", "pref")?.filler).toBe("tabs");
    expect(s2.turns()).toContain("we decided to use pgvector for the durable stator");
    expect(s2.sessionOrdinal("sess-A")).toBe(0);
    // A new session continues the ordinal sequence, not restart from 0.
    expect(s2.touchSession("sess-B")).toBe(1);
    await s2.flush();
  });
});
...
describe("pgvector stator — event history, cache and kv are durable", () => {
  it("hydrates the run tape, result cache and kv across a restart", async () => {
    const db = await pglite();
    const s1 = await PgVectorStore.create({ client: db });
    s1.history.append(rec("r1", "a", 0));
    s1.history.append(rec("r1", "a", 1)); // a retry
    expect(s1.history.lastAttempt("r1", "a")).toBe(1);
    expect(s1.history.lookup("r1", "a", 0)).toBeDefined();
    s1.cache.put("k1", { answer: 42 }, { scope: "rotor" });
    s1.cache.put("k2", { v: 1 }, { scope: "rotor", ttlTicks: 5 });
    expect(s1.cache.get("k1", 0)).toEqual({ answer: 42 });
    expect(s1.cache.get("k2", 9)).toBeUndefined(); // expired past its tick
    s1.kvSet("kk", { hello: "world" });
    expect(s1.kvGet("kk")).toEqual({ hello: "world" });
    await s1.flush();

    const s2 = await PgVectorStore.create({ client: db });
    expect(s2.history.read("r1").length).toBe(2);
    expect(s2.history.lastAttempt("r1", "a")).toBe(1);
    expect(s2.cache.get("k1", 0)).toEqual({ answer: 42 });
    expect(s2.kvGet("kk")).toEqual({ hello: "world" });
    await s2.flush();
  });
});
...
describe("pgvector stator — lifecycle", () => {
  it("shutdown flushes then releases the client", async () => {
    ...
    store.addTurn("persisted before shutdown");
  ...
  it("flush surfaces a persistence error instead of swallowing it", async () => {
    ...
    store.writeFacts([{ entity: "a", role: "b", filler: "c", is_current: true, tick: 0 }]);
    // Mirror still has it (control path unaffected); the durability error surfaces on flush.
    expect(store.lookupFact("a", "b")?.filler).toBe("c");
...
describe("pgvector stator — ANN recall in the database", () => {
  it("ranks turns by cosine similarity via the vector index", async () => {
    ...
    store.addTurn("the config parser reads parseConfig from the yaml file");
    store.addTurn("the pool warms hot instances up to the budget");
    store.addTurn("the drain forwards step records to an external sink");
    await store.flush();
...
describe("pgvector stator — dim-cap degrades to an exact scan", () => {
  it("an embedding dim above the index cap is unindexed but still recalls", async () => {
    ...
    store.addTurn("alpha beta gamma delta");
    store.addTurn("completely unrelated words here");
    await store.flush();
