...
| **Secrets isolation** (§12.3) | a step names a **connection**, never a secret | **custom-MCP / connector** model — connection headers are stored **write-only + encrypted** and executed **server-side** by the glyphh server; the secret never enters model context or a `StepRecord`. |
